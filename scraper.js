...
async function scrapeDays(courseName, courseUrl, db, daysToScrape = 5) {
  ...
  for (let i = 0; i < daysToScrape; i++) {
    const currentDateText = await page.$eval(".dateFormat", (el) =>
      el.innerText.trim()
    );
    console.log(
    ...
    try {
      await page.waitForSelector(".tee-time-slot, .teeTime.ng-star-inserted", {
        timeout: 3000,
      });
      ...
      const teeTimes = await page.evaluate(() => {
        ...
        return Array.from(
          container.querySelectorAll(".teeTime.ng-star-inserted")
        )
          .map((slot) => {
            const time = slot.querySelector(".timeCol")?.innerText?.trim();
            const availableRaw = slot
              .querySelector(".availableBookings")
              ?.innerText?.trim();
            ...
            if (playersRaw && playersRaw.includes("-")) {
              [minPlayers, maxPlayers] = playersRaw
                .split("-")
    ...
    // Move to next day
    const rightChevron = await page.$(
      '.dateNavigation img[src*="chevron-right"]'
    );
    if (!rightChevron) {
      console.warn(
        `${courseName} - Could not find "Next Day" chevron. Stopping.`
      );
      break;
    }
